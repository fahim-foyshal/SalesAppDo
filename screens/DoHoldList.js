...
const DoHoldList = () => {
  const dispatch = useDispatch();
    ...
    const doHoldData = useSelector(state => state.doholdinformationdata.doHoldData);
    console.log(doHoldData);

  const  handlepressforgetorder = (item)=>{
    navigation.navigate('GetOrder', { timestamp:item.do_no ,dealercode:item.dealer_code,shopName:item.shop_name});
  }
